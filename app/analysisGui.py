...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        buttons.append(tk.Button(self, text='Export', command = lambda: [os.chdir('{}'.format(home_folder)),pathlib.Path('exports').mkdir(parents=True, exist_ok=True), controller.percents.to_csv('percents.csv'), controller.values.to_csv('counts.csv'),[x.figure.savefig('{}.png'.format(x.figure.texts[0].get_text[0])) for x in controller.panels]]))
